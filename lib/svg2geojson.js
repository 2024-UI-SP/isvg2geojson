...
function convertSVGToGeoJSON(doc, options={}) {
	...
	for (var name in layerByName) {
		...
		const geo = {
			...
			features:layerByName[name].map(data => {
				convertCoordinates(data.coordinates, options.image); // RLP mods: change 'converter' to 'options.image'. I'm not sure why there ever was a 'converter' variable
				return {
					type:'Feature',
	...
	/**
	 * RLP modifies by adding bolean argument, image, to flip the y-axis
	 */
	function convertCoordinates(coords, image=false) {
		if (typeof coords[0]==='number') {
			converter(coords);
			// RLP adds
			if (image) {
				const imageHeight = corners[1].$.Y.value - corners[0].$.Y.value; 
				coords[1] = imageHeight - coords[1]; 
			}
			// end RLP adds
		}
		else coords.forEach( a => { convertCoordinates(a, image) } ); // RLP mods
		return coords;
	}
}
